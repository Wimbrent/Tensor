...
class TestImages:
    ...
    def test_images(self, Keyword):
        ...
        with allure.step("Проверить url страницы"):
            assert driver.current_url == imagespage_url
        time.sleep(2)
        ...
        with allure.step("Сравнить название категории с текстом из поля поиска"):
            assert yandex_images.get_text_image(Keyword) == yandex_images.get_text_from_search_field()
        time.sleep(3)
        ...
        with allure.step("Сравнить текст"):
            assert yandex_images.first_assert_image() == yandex_images.second_assert_image()